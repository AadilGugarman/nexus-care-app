...
export async function createDoctorCreationRequest(
  ...
  const { data, error } = await requestTables.creation()
    .insert({
      ...input,
...
export async function getDoctorCreationRequests(
  ...
  let query = requestTables.creation()
    .select(`
      *,
...
export async function approveDoctorCreationRequest(
  ...
  const { data: updatedRequest, error: updateError } = await requestTables.creation()
    .update({
      status: 'approved',
...
export async function createDoctorChangeRequest(
  ...
  const { data, error } = await requestTables.change()
    .insert({
...
export async function getDoctorChangeRequests(
  ...
  let query = requestTables.change()
    .select(`
      *,
      doctor:doctors!doctor_change_requests_doctor_id_fkey(name),
  ...
  return ((data as any[]) || []).map((item: any) => ({
    ...item,
    doctor_name: item.doctor?.name || 'Unknown Doctor',
    requester_name: item.requester?.full_name || null,
    requester_email: item.requester?.email || null,
    reviewer_name: item.reviewer?.full_name || null,
    reviewer_email: item.reviewer?.email || null,
...
export async function getDoctorChangeRequestById(
  ...
  return {
    ...result,
    doctor_name: result.doctor?.name || 'Unknown Doctor',
    requester_name: result.requester?.full_name || null,
    requester_email: result.requester?.email || null,
    reviewer_name: result.reviewer?.full_name || null,
    reviewer_email: result.reviewer?.email || null,
...
export async function createDoctorStatusRequest(
  ...
  const { data, error } = await requestTables.status()
    .insert({
      doctor_id: input.doctor_id,
...
export async function getDoctorStatusRequests(
  ...
  let query = requestTables.status()
    .select(`
      *,
      doctor:doctors!doctor_status_requests_doctor_id_fkey(name, is_active),
...
export async function getRequestStatistics(): Promise<RequestStatistics> {
  ...
  const [creationStats, changeStats, statusStats] = await Promise.all([
    requestTables.creation()
      .select('status', { count: 'exact', head: true }),
    requestTables.change()
      .select('status', { count: 'exact', head: true }),
    requestTables.status()
      .select('status', { count: 'exact', head: true }),
  ]);
  ...
  const getStatusCounts = async (table: 'creation' | 'change' | 'status') => {
    ...
    const [pending, approved, rejected, total] = await Promise.all([
      tableRef().select('*', { count: 'exact', head: true }).eq('status', 'pending'),
      tableRef().select('*', { count: 'exact', head: true }).eq('status', 'approved'),
      tableRef().select('*', { count: 'exact', head: true }).eq('status', 'rejected'),
  ...
  const [creation_requests, change_requests, status_requests] = await Promise.all([
  ...
  return {
    ...
    status_requests,
    total_pending:
      creation_requests.pending + change_requests.pending + status_requests.pending,
  };
}
